...
class ToReadItem(BaseItem):
    def __init__(self, heading, due_date, url_adress):
        super(ToReadItem, self).__init__(heading)
        self.due_date = due_date
        self.url_adress = url_adress

    def __str__(self):
        return '{} ToReadItem: {} for {} {}'.format(
            '+' if self.done else '-',
            self.heading,
            self.due_date,
            self.url_adress,
    ...
    def construct(cls):
        ...
        url_adress = input_function('input url_adress: ')
        return ToReadItem(heading, due_date, url_adress)